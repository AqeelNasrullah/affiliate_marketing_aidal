...
router.post("/search", async (req,res)=>{
  try{
    const items = await Item.find({name : {'$regex' : req.body.search , '$options' : 'i'}},(err)=>{
      if(err){
        console.log(err)
      }
    }).sort({id:-1})
...
router.post("/add",verify,upload.single("image"),async (req,res)=>{
  ...
  await Count.findOneAndUpdate({id : 1},{total : status.total + 1,current_id : status.current_id + 1 },function(err){
    if(err){
      res.send(err);
      return;
    }
